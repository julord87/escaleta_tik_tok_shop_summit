        ...
        {selectedDay && <div className="mt-6">
          ...
          <div className="mt-5 grid gap-8 lg:grid-cols-[minmax(0,1fr)_290px]">
            <div className="overflow-hidden border border-border-strong">
              <div className="hidden bg-surface-2 sm:grid sm:grid-cols-[82px_150px_minmax(0,1fr)]"><span className="border-r border-border-strong px-3 py-2 font-mono text-[9px] font-semibold uppercase tracking-[0.12em] text-text-dim">Hora</span><span className="border-r border-border-strong px-3 py-2 font-mono text-[9px] font-semibold uppercase tracking-[0.12em] text-text-dim">Espacio</span><span className="px-3 py-2 font-mono text-[9px] font-semibold uppercase tracking-[0.12em] text-text-dim">Actividad</span></div>
              {orderedTasks.length === 0 ? <p className="px-3 py-5 text-[13px] text-text-dim">Todavía no hay actividades cargadas para este día.</p> : orderedTasks.map(({ lane, task }, index) => <article key={task.id} className={`grid sm:grid-cols-[82px_150px_minmax(0,1fr)] ${index > 0 ? 'border-t border-border' : ''}`}><time className="border-b border-border px-3 py-3 font-mono text-[11px] font-bold tabular-nums text-text sm:border-b-0 sm:border-r sm:border-border-strong">{task.time_label || '—'}</time><div className="border-b border-border px-3 py-2.5 font-mono text-[9px] font-bold uppercase leading-relaxed tracking-[0.1em] text-text sm:border-b-0 sm:border-r sm:border-border-strong">{lane.room}<br /><span className="font-normal text-text-faint">{lane.floor}</span></div><div className="px-3 py-3"><h3 className={`text-[13px] leading-snug ${task.checked ? 'text-text-faint line-through' : task.variant === 'accent' ? 'font-bold text-text' : 'font-semibold text-text'}`}>{task.what}</h3>{(task.meta || task.who) && <p className="mt-0.5 font-mono text-[10px] text-text-faint">{[task.meta, task.who].filter(Boolean).join(' · ')}</p>}{task.note && <p className="mt-1 border-l-2 border-border-strong pl-2 text-[11.5px] italic text-text-dim">{task.note}</p>}</div></article>)}
            </div>
