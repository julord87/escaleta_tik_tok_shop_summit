        ...
        {selectedDay && <div className="mt-6">
          ...
          <div className="mt-5 grid gap-8 lg:grid-cols-[minmax(0,1fr)_290px]">
            <div className="border-t-2 border-text">
              {orderedTasks.length === 0 ? <p className="py-5 text-[13px] text-text-dim">Todavía no hay actividades cargadas para este día.</p> : orderedTasks.map(({ lane, task }) => <article key={task.id} className={`grid gap-x-3 border-b border-border py-3 sm:grid-cols-[74px_116px_minmax(0,1fr)] ${task.variant === 'accent' ? 'bg-yellow px-3' : ''}`}><time className="font-mono text-[11px] font-bold tabular-nums text-text">{task.time_label || '—'}</time><div className="font-mono text-[9px] uppercase tracking-[0.1em] text-text-faint">{lane.room}<br />{lane.floor}</div><div><h3 className={`text-[13px] leading-snug ${task.checked ? 'text-text-faint line-through' : 'font-semibold text-text'}`}>{task.what}</h3>{(task.meta || task.who) && <p className="mt-0.5 font-mono text-[10px] text-text-faint">{[task.meta, task.who].filter(Boolean).join(' · ')}</p>}{task.note && <p className="mt-1 border-l-2 border-border-strong pl-2 text-[11.5px] italic text-text-dim">{task.note}</p>}</div></article>)}
            </div>
